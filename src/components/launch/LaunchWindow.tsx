				...
				<div
					...
					title={
						recording
							...
							: "Recording container format. WebM = smaller, MP4 = universal."
					}
				>
					{(["webm", "mp4"] as const).map((fmt) => (
						<button
							key={fmt}
							className={`px-3 py-1.5 rounded-full text-[11px] font-mono font-semibold uppercase tracking-wider transition-colors ${
								recordingFormat === fmt
									? "bg-white/15 text-white drop-shadow-[0_0_4px_rgba(52,199,123,0.35)]"
									: "text-white/45 hover:text-white/80 hover:bg-white/5"
							} ${recording ? "cursor-not-allowed opacity-60" : ""}`}
							onClick={() => !recording && setRecordingFormat(fmt)}
							disabled={recording}
						>
							{fmt === "mp4" ? "MP4" : "WebM"}
						</button>
